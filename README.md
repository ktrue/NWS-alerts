...
The following is from Curly's original _NWS-alerts_read_me.html_ converted to Markdown for this README file.
...
*   A Google map displays an outlined warning area
...
**nws-shapefile.txt** - This file is provided by the NWS. It contains all of the zone codes with their latitude and longitude. This file is used for getting the locations for the Google map that is placed in the details page.
...
<span style="color: #960;">$googleAPI = '-replace-this-with-your-API-key-here-';    // your Google API key;</span>  
Enter your Google API key to display the alert map. If you leave the code as is, the map will not display.  
If this setting is in the file Settings.php, you do not need to change this as it will be overwritten.

<span style="color: #960;">$zoomLevel = '7';    // default zoom level</span>  
...
<span style="color: #960;">$mapStyle= 4;    // google map style  
</span>   // 1 = ROADMAP displays the normal, default 2D tiles of Google Maps.  
   ...
   // 4 = TERRAIN displays physical relief tiles for displaying water features (mountains, rivers, etc.).  
Selectable default map type for the Google map.  
...
NOTE: If a Zone code is the first code entered, the Google map will display in the details page. If a County code is the first code entered, the Google map will not display.
...
It does not display -  
    ...
    • $displaymap is not set to 1 in the file nws-alerts-config.php.  
    • A zone code for a location is not in the nws-shapefile.txt file.  
      This file is supplied by the NWS and not all locations are listed.
